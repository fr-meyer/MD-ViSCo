...
@dataclass
class PulseDBApproximationNabNetConfig(PulseDBBaseConfig):
    """PulseDB dataset configuration for NABNet model"""
    args: dict = None
    ...
    def _train_epoch(self, epoch, ddp_model, train_loader, optim, device, master_process, 
        ...
        rank = device.index
        print(f"Rank {rank} - Starting training epoch {epoch}")
            ...
            try:
                ...
                if hasattr(self, 'extract_waveform_features') and self.extract_waveform_features:
                    ...
                    if target.lower() not in ['ecg', 'ppg']:
                        print(f"\nSkipping {recon_key} feature extraction: Target signal {target} is not ECG or PPG")
                        return
                    ...
                    with torch.no_grad():
                        ...
                        features_dir = os.path.join('./results/features_extraction', 'PulseDB', 'NABNet', f'seed_{self.seed}')
