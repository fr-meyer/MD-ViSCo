...
from baseline.P2E_WGAN import GeneratorUNet, Discriminator, weights_init_normal, compute_gradient_penalty
from utils.ddp_utils import create_ddp_dataloaders, print_memory_stats
...
@dataclass
class UCIApproximationP2EWGANConfig(UCIBaseConfig):
    """UCI dataset configuration for P2E-WGAN model.
    ...
    generator_init_filters: int = 288  # Controls generator network capacity
    discriminator_init_filters: int = 144  # Controls discriminator network capacity
    ...
    def _prepare_batch(self, batch_data, device, source_channel, target_channel):
        ...
        # Update batch data indexing for UCI dataset
        x = batch_data[0][:, source_channel:source_channel + 1].to(device, torch.float32)
        y_target = batch_data[0][:, target_channel:target_channel + 1].to(device, torch.float32)
    ...
    def _train_epoch(self, epoch, ddp_generator, ddp_discriminator, train_loader, 
        ...
        with tqdm(train_loader, unit="batch", ncols=125, disable=not master_process) as tepoch:
            ...
            for step, train_file in enumerate(tepoch):
                ...
                if step % self.n_critic == 0:
                    ...
                    lambda_sample = 50  # Same value as in your original script
                    ...
                    # Add synchronization barrier after generator update to ensure all processes are in sync
                    dist.barrier()
                ...
                # Add synchronization barrier after discriminator update to ensure all processes are in sync
                dist.barrier()
                ...
                if master_process and wandb.run is not None:
                    try:
                        wandb.log({
                            ...
                            "train/g_mse_loss": mse_loss.item() if step % self.n_critic == 0 else None,
                            "train/g_mae_loss": mae_loss.item() if step % self.n_critic == 0 else None,
                ...
                tepoch.set_postfix(
                    ...
                    g_loss=f"{g_loss.item():.4f}" if step % self.n_critic == 0 else "N/A",
                    g_mse=f"{mse_loss.item():.4f}" if step % self.n_critic == 0 else "N/A",
                    g_mae=f"{mae_loss.item():.4f}" if step % self.n_critic == 0 else "N/A"
                )
        ...
        return avg_g_loss, avg_d_loss, avg_g_mse, avg_g_mae
    ...
    # Include other necessary methods from PulseDBBaseConfig
    ...
    def get_checkpoint_path(self, source, target, epoch: int = None, saving: bool = False):
        ...
        sub_folder = f'AppModel_UnetWGAN_BS_{self.batch_size}_E_{self.num_epochs}_LR_{self.learning_rate}_P_{self.scheduler_patience}_ES_{self.early_stopping_patience}'
                ...
                if rank == 0:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            # Load discriminator state dict (only on rank 0)
                            disc_state_dict = checkpoint['discriminator_state_dict']
                            # Remove 'module.' prefix if it exists in the checkpoint
                            new_disc_state_dict = {k.replace('module.', ''): v 
                    ...
                    print(f"Rank {rank} - Waiting for checkpoint loading decision from rank 0")
            else:
                if rank == 0:
            ...
            for e in range(epoch_checkpoint, self.num_epochs):
                ...
                if master_process:
                    ...
                    prev_lr = self._update_training_state(
                        e, val_mae, ddp_generator, ddp_discriminator,
                    ...
                    if wandb.run is not None:
                        try:
                            log_dict = {
                                "epoch": e,
                                "train/epoch_g_loss": float(train_g_loss),
                                "train/epoch_d_loss": float(train_d_loss),
                                "train/epoch_mse": float(train_g_mse),
                                "train/epoch_mae": float(train_g_mae),
    ...
    def test(self, num_workers:int):
        ...
        # Initialize metrics dictionaries for all directions
        mae_metrics = {}
        correlation_metrics = {}
        ...
        for source_channel, target_channel in source_target_pairs:
            ...
            recon_key = f"{self.channel_names[source_channel].lower()}2{self.channel_names[target_channel].lower()}"
            mae_metrics[f"{recon_key}_loss"] = []
            # Initialize correlation metric with proper prefix
            correlation_metrics[f"pC_{recon_key}_loss"] = []

            print(f"\nTesting {recon_key} model...")
            ...
            try:
                ...
                generator.load_state_dict({k.replace('module.', ''): v 
                                        for k, v in checkpoint['generator_state_dict'].items()})
                generator = generator.to(device)
                generator.eval()

                # Print model parameters
                print(f"\nModel Parameters for {recon_key}:")
                print_model_parameters(generator)
                ...
                if hasattr(self, 'extract_waveform_features') and self.extract_waveform_features:
                    print(f"Extracting waveform features from {recon_key} reconstructed signals...")
                    
                    # Check if target is ECG or PPG
                    target_name = self.channel_names[target_channel].lower()
                    if target_name not in ['ecg', 'ppg']:
                        print(f"\nSkipping {recon_key} feature extraction: Target signal {target_name} is not ECG or PPG")
                        return
                    ...
                    with torch.no_grad():
                        ...
                        features_dir = os.path.join('./results/features_extraction', 'UCI', 'P2EWGAN', f'seed_{self.seed}')
                        filename = f'features_{recon_key}.h5'
                        filepath = os.path.join(features_dir, filename)
                        
                        try:
                            check_file_exists(filepath)
                        except FileExistsError as e:
                            print(f"\nSkipping {recon_key} feature extraction: {e}")
                            return
                        
                        with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                            tepoch_test.set_description(f"Extracting {recon_key} Features")
                            
                            for batch_idx, test_file in enumerate(tepoch_test):
                                # Get source and target signals
                                source_signal = test_file[0][:, source_channel:source_channel + 1].to(device, torch.float32)
                                target_signal = test_file[0][:, target_channel:target_channel + 1].to(device, torch.float32)
                                
                                # Get reconstruction
                                recon = generator(source_signal)
                                ...
                                with tqdm(range(batch_size), unit="sample", ncols=100, leave=False) as pbar:
                                    ...
                                    for i in pbar:
                                        # Use batch index as the key since UCI dataset doesn't return global indices
                                        sample_idx = batch_idx * self.test_batch_size + i
                                        
                                        # Extract features for this sample
                                        features = extract_waveform_features(
                                            ecg_signal=Min_Max_Norm_Torch(recon[i:i+1, :, :])[0, 0, :].cpu().numpy() if target_name == 'ecg' else None,
                                            ppg_signal=Min_Max_Norm_Torch(recon[i:i+1, :, :])[0, 0, :].cpu().numpy() if target_name == 'ppg' else None
                                        )
                                        
                                        # Store features using sample index as key
                                        direction_features[sample_idx] = features
                                        
                                        # Update inner progress bar
                                        pbar.set_postfix({
                                            'sample': f"{i+1}/{batch_size}",
                                            'idx': sample_idx,
                                            'features': '✓' if features[f'{target_name}_features'] is not None else '✗'
                                        })
                    ...
                    with torch.no_grad():
                        with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                            tepoch_test.set_description(f"Testing {recon_key}")
                            ...
                            for batch_idx, test_file in enumerate(tepoch_test):
                                ...
                                recons = {recon_key: fake_B}
                                targets = {self.channel_names[target_channel].lower(): batch["y_target"]}
                                
                                # Calculate losses
                                losses, batch_losses = calculate_reconstruction_losses(recons, targets, l1)
                                
                                # Calculate Pearson correlations with proper prefix
                                correlations = calculate_pearson_correlations(recons, targets, correlation_metrics)
                                
                                # Update mae_metrics dictionary
                                mae_metrics[f"{recon_key}_loss"].extend(batch_losses[f"{recon_key}_loss"])
                                ...
                                if bpm_metrics is not None and ground_truth_rates is not None:
                                    ...
                                    tepoch_test.set_postfix({
                                        "mae": f"{torch.mean(losses[recon_key].cpu()):.4f}",
                                        "corr": f"{correlations[recon_key]:.4f}" if recon_key in correlations else "0.0000",
                                ...
                                # Clear memory
                                del batch, fake_B, losses, batch_losses, correlations
                                if ground_truth_rates is not None:
                                    del ground_truth_rates
                                torch.cuda.empty_cache()

            except Exception as e:
                print(f"Error testing {recon_key} model: {str(e)}")
                continue
        ...
        # Print final results for all directions
        print("\nMAE Results:")
