...
submodules recursively, with proper error handling and logging.
"""
...
def _iter_submodules(package_name: str) -> Iterable[str]:
    ...
    Yields:
        Fully qualified module names (e.g., "src.trainers.approximation_trainer")
    """
    try:
...
def import_modules(package_name: str, module_type: str = "modules") -> int:
    ...
    Args:
        package_name: The package to scan (e.g., "src.trainers")
        module_type: Human-readable name for logging (e.g., "trainers", "models", "criterions")
    ...
    log.info(f"Successfully imported {imported_count} {module_type} modules" + 
