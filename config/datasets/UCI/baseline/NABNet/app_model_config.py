...
from baseline.NABNet_pytorch import NABNetV2
...
@dataclass
class UCIApproximationNabNetConfig(UCIBaseConfig):
    """UCI dataset configuration for NABNet model"""
    args: dict = None
    ...
    def create_model(self):
        """Create NABNet model instance"""
        return NABNetV2(
            length=self.input_size,
            model_depth=self.model_depth,
            num_channel=self.in_channels,
            model_width=self.model_width,
            kernel_size=self.kernel_size,
            problem_type='Regression',
            output_nums=self.out_channels,
            ds=self.D_S,
            ae=self.A_E,
            ag=self.A_G,
            lstm=self.LSTM,
        ...
        with tqdm(train_loader, unit="batch", ncols=125, disable=not master_process) as tepoch:
            ...
            for step, train_file in enumerate(tepoch):
                ...
                if isinstance(outputs, list):
                    # Calculate loss for each supervision level
                    mse_losses = []
                    mae_losses = []
                    for i, output in enumerate(outputs):
                        # Resize output to match target size if needed
                        if output.shape != batch["y_target"].shape:
                            output = torch.nn.functional.interpolate(
                                output,
                                size=batch["y_target"].shape[-1],
                                mode='linear',  # Use linear interpolation for 1D signals
                                align_corners=False
                            )
                        
                        # Verify shapes match after potential resizing
                        assert output.shape == batch["y_target"].shape, \
                            f"Shape mismatch at level {i}: output {output.shape} != target {batch['y_target'].shape}"
                        
                        # Calculate weighted MSE loss for this level
                        level_mse = torch.nn.functional.mse_loss(
                            batch["y_target"],
                            output
                        ) * (1 - i * 0.1)  # Decreasing weights for deeper levels

                        with torch.no_grad():
                            level_mae = torch.nn.functional.l1_loss(
                                batch["y_target"],
                                output.detach()
                            ) * (1 - i * 0.1)
                        
                        mse_losses.append(level_mse)
                        mae_losses.append(level_mae)
                    mse_loss = sum(mse_losses)
                    mae_loss = sum(mae_losses)
                ...
                for val_file in tepoch_val:
                    ...
                    if isinstance(outputs, list):
                        ...
                        for i, output in enumerate(outputs):
                            ...
                            batch_mse_loss += level_mse
                            batch_mae_loss += level_mae
                    else:
                ...
                if rank == 0:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            # Load optimizer and scheduler states on rank 0
    ...
    def _update_training_state(self, epoch, val_loss, ddp_model, optim, scheduler,
                               early_stopping, checkpoint_path, best_loss, prev_lr, source, target):
        ...
        if best_loss['best_recon_total_loss'] is None or best_loss['best_recon_total_loss'] > val_loss:
            torch.save({
                ...
                'epoch': int(epoch+1),  # Save epoch+1 so training resumes correctly
        ...
        if epoch % 5 == 0:
            ...
            checkpoint_info_epoch = self.get_checkpoint_path(source, target, epoch, saving=True)
    ...
    def get_source_target_pairs(self):
        ...
        signals = [self.ecg_label, self.ppg_label, self.abp_label]  # Add ECG to signals
        
        # Generate all valid pairs (source, target)
        pairs = []
        for source in signals:
            for target in signals:
                if source != target:  # Exclude identity mappings
    ...
    def get_checkpoint_path(self, source, target, epoch: int = None, saving: bool = False):
        ...
        sub_folder = f'AppModel_NabNet_BS_{self.batch_size}_E_{self.num_epochs}_LR_{self.learning_rate}_P_{self.scheduler_patience}_ES_{self.early_stopping_patience}'
    ...
    def _train_single_direction(self, dataset: tuple, rank: int, world_size: int, ddp_config, source_channel, target_channel):
        ...
        try:
            ...
            # Create model without DDP wrapping first
            ...
            # Load checkpoint if exists - before DDP wrapping
            training_state = self.load_checkpoint_with_retry(
                rank=rank,
                model=model,  # Unwrapped model
                optim=optim,
                scheduler=scheduler,
            ...
            for e in range(epoch_checkpoint, self.num_epochs):
                # Pass source and target channels to training functions
            ...
            try:
                ...
                if hasattr(self, 'extract_waveform_features') and self.extract_waveform_features:
                    ...
                    with torch.no_grad():
                        ...
                        features_dir = os.path.join('./results/features_extraction', 'UCI', 'NABNet', f'seed_{self.seed}')
                        ...
                        with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                            ...
                            for batch_idx, test_file in enumerate(tepoch_test):
                                ...
                                # Get reconstruction
                                outputs = model(source_signal)
                                
                                # Handle deep supervision outputs if enabled
                                if isinstance(outputs, list):
                                    recon = outputs[0]  # Use the first (main) output
                                else:
                                    recon = outputs
                                ...
                                with tqdm(range(batch_size), unit="sample", ncols=100, leave=False) as pbar:
                                    ...
                                    for i in pbar:
                                        # Calculate global index based on batch index and batch number
                    ...
                    with torch.no_grad():
                        with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                            ...
                            for batch_idx, test_file in enumerate(tepoch_test):
                                ...
                                # Prepare batch data
                                x = test_file[0][:, source_channel:source_channel + 1].to(device, torch.float32)
                                y_target = test_file[0][:, target_channel:target_channel + 1].to(device, torch.float32)
                                ...
                                # Handle deep supervision outputs if enabled
                                if isinstance(outputs, list):
                                    output = outputs[0]  # Use the first (main) output
                                else:
                                    output = outputs
                                
                                # Format data for helper functions
                                recons = {recon_key: output}
                                targets = {self.channel_names[target_channel].lower(): y_target}
                                ...
                                # Clear memory
                                del x, y_target, outputs, output, recons, targets, losses, batch_losses, correlations
