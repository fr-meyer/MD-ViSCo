...
from .app_model_config import UCIApproximationNabNetConfig

@dataclass
class UCIRefinementNabNetConfig(UCIBaseConfig):
    ...
    model_depth: int = 1
    model_width: int = 112 # Increased from 84 to 112 to match the number of parameters in reference model
    ...
    def test(self, num_workers: int):
        ...
        """
        app_model_config = UCIApproximationNabNetConfig(args=self.args)
        ...
        for direction in directions:
            ...
            checkpoint_info = self.get_checkpoint_info(is_finetuning=self.is_finetuning, saving=False)
            ...
            try:
                ...
                g_model_app.eval()
                print(f'\nLoaded approximation model checkpoint from {app_checkpoint_info["file"]} for {direction}')

                # Load refinement model checkpoint
                checkpoint = torch.load(checkpoint_info['file'])
                model.load_state_dict({k.replace('module.', ''): v for k, v in checkpoint['model_state_dict'].items()})
                model.eval()
                print(f'Loaded refinement model checkpoint from {checkpoint_info["file"]} for {direction}')
            ...
            with torch.no_grad():
                with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                    ...
                    for test_file in tepoch_test:
                        # un-normalized abp waveform
                        x_abp_u = test_file[10].to(device, torch.float32)
                        
                        # Prepare batch data
                        batch = {
                            "x": test_file[0].to(device, torch.float32),
                            "y_target": x_abp_u.to(device, torch.float32),
                            "sbp": test_file[2][:, 0:1].to(device, torch.float32),
                            "dbp": test_file[2][:, 1:2].to(device, torch.float32)
                        }
                        ...
                        if isinstance(x_abp, list):
                            x_abp = Min_Max_Norm_Torch(x_abp[0][:, :, :])
                        else:
                            x_abp = Min_Max_Norm_Torch(x_abp[:, :, :])
