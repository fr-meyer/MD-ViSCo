...
from scipy.signal import (
    kaiserord, firwin, filtfilt, detrend, 
    periodogram, lfilter, find_peaks, firls, resample
)
...
def extract_ppg_features(ppg_signal, fs, start_sig=0, end_sig=-1):
    """
    Extract fiducial points, biomarkers and SQI from a PPG signal using pyPPG.
    
    Args:
        ppg_signal (np.array): Raw PPG signal
        fs (float): Sampling frequency in Hz
        start_sig (int): Start sample for analysis (default: 0)
        end_sig (int): End sample for analysis (default: -1 for end of signal)
        
    Returns:
        dict: Dictionary containing:
            - fiducials: DataFrame with fiducial points
            - biomarkers: Dictionary with biomarker values and statistics
            - sqi: Signal Quality Index value
    """
    # Ensure ppg_signal is a numpy array
    ppg_signal = np.array(ppg_signal[:], dtype=np.float64)
    
    # Create signal object with proper initialization using DotMap
    signal_dict = {
        'v': ppg_signal,
        'fs': float(fs),
        ...
        'correction': pd.DataFrame()  # Initialize empty correction DataFrame
    }
    ...
    # Create DotMap from dictionary
    signal = DotMap(signal_dict)
    
    # Create PPG object
    s = PPG(signal)
    
    # Initialize Preprocess with the same parameters as in the signal
    prep = PP.Preprocess(
        fL=s.fL,
        fH=s.fH,
        order=s.order,
    ...
    try:
        # Create FpCollection instance using the local class
        fpex = FpCollection(s=s)
        
        fiducials = fpex.get_fiducials(s=s)
        
        # Create Fiducials object
        ...
        return {
            'Asp_deltaT': np.mean(df_biomarkers_sig['Asp/deltaT']),
            'IPR': np.mean(df_biomarkers_sig['IPR'])
            }
    except Exception as e:
        print(f"Error in fiducial point extraction: {str(e)}")
...
def _check_shape_(signal, fs):
    assert len(signal) >= fs*8, "Signal must be at least eight seconds"
