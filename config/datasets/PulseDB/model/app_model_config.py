...
@dataclass
class PulseDBApproximationConfig(PulseDBBaseConfig):
    """PulseDB dataset configuration for Approximation model"""
    args: dict = None
    ...
    def __post_init__(self):
        ...
        self.path_folder: str = 'AppModel/PulseDB'
    ...
    def _prepare_batch(self, batch_data, device):
        ...
        if self.direction is not None:
            ...
            x_real = batch_data[8][:, self.source_channel:self.source_channel + 1].to(device, torch.float32)
            x_real_target = batch_data[8][:, self.target_channel:self.target_channel + 1].to(device, torch.float32)
            ...
            batch_size = batch_data[8].size(0)
            ...
            x_real = batch_data[8][:, rand_origin:rand_origin + 1].to(device, torch.float32)
            x_real_target = batch_data[8][:, rand_target:rand_target + 1].to(device, torch.float32)
        ...
        with torch.no_grad():
            with tqdm(val_loader, unit="batch", ncols=125, disable=not master_process) as tepoch_val:
                ...
                for val_file in tepoch_val:
                    if self.direction is not None:
                        # Uni-directional validation
                        input_signal = val_file[8][:, self.source_channel:self.source_channel + 1].to(device, torch.float32)
                        target_signal = val_file[8][:, self.target_channel:self.target_channel + 1].to(device, torch.float32)
                        ...
                        input_ii = val_file[8][:, self.ecg_label:self.ecg_label + 1].to(device, torch.float32)
                        input_ppg = val_file[8][:, self.ppg_label:self.ppg_label + 1].to(device, torch.float32)
                        input_abp = val_file[8][:, self.abp_label:self.abp_label + 1].to(device, torch.float32)
                        ...
                        output_ii = val_file[8][:, self.ecg_label:self.ecg_label + 1].to(device, torch.float32)
                        output_ppg = val_file[8][:, self.ppg_label:self.ppg_label + 1].to(device, torch.float32)
                        output_abp = val_file[8][:, self.abp_label:self.abp_label + 1].to(device, torch.float32)
        ...
        if hasattr(self, 'extract_waveform_features') and self.extract_waveform_features:
            ...
            for direction, signal_info in directions.items():
                ...
                with torch.no_grad():
                    ...
                    features_dir = os.path.join('./results/features_extraction', 'PulseDB', 'MD-ViSCo', f'seed_{self.seed}')
                    ...
                    with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                        ...
                        for batch_idx, test_file in enumerate(tepoch_test):
                            ...
                            source_signal = test_file[8][:, getattr(self, f"{signal_info['source']}_label"):getattr(self, f"{signal_info['source']}_label") + 1].to(device, torch.float32)
                            target_signal = test_file[8][:, getattr(self, f"{signal_info['target']}_label"):getattr(self, f"{signal_info['target']}_label") + 1].to(device, torch.float32)
                            ...
                            with tqdm(range(batch_size), unit="sample", ncols=100, leave=False) as pbar:
                                ...
                                for i in pbar:
                                    ...
                                    features = extract_waveform_features(
                                        ecg_signal=Min_Max_Norm_Torch(recon[i:i+1, :, 15:-15])[0, 0, :].cpu().numpy() if signal_info['target'] == 'ecg' else None,
                                        ppg_signal=Min_Max_Norm_Torch(recon[i:i+1, :, 15:-15])[0, 0, :].cpu().numpy() if signal_info['target'] == 'ppg' else None
                                    )
            ...
            with torch.no_grad():
                with tqdm(test_loader, unit="batch", ncols=125) as tepoch_test:
                    ...
                    for batch_idx, test_file in enumerate(tepoch_test):
                        ...
                        # Get ground truth rates from dataset
                        ...
                        if self.direction is not None:
                            ...
                            input_signal = test_file[8][:, source_label:source_label + 1].to(device, torch.float32)
                            target_signal = test_file[8][:, target_label:target_label + 1].to(device, torch.float32)
                            ...
                            # Prepare inputs
                            inputs = {
                                'ecg': test_file[8][:, ii_label:ii_label + 1].to(device, torch.float32),
                                'ppg': test_file[8][:, ppg_label:ppg_label + 1].to(device, torch.float32),
                                'abp': test_file[8][:, abp_label:abp_label + 1].to(device, torch.float32)
                            }
